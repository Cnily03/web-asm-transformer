!function () {
    ...
    function wrapWithSpan(str, color) {
    ...
    function getIOSpan(type, value) {
        ...
        if (type === 'error' || type === 'warn') {
            let prefix = type === 'error' ? 'Error: ' : 'Warning: '
            let topSpan = document.createElement('span')
            topSpan.appendChild(wrapWithSpan(prefix, color_map[type]))
            topSpan.appendChild(wrapWithSpan(value, color_map['log']))
            return topSpan
        } else return wrapWithSpan(value, color_map[type])
    }

    class ConsoleIO {
        /**
         * @param {(type, value) => any} receiveFunc
         */
        constructor(receiveFunc) {
            this.output_args = []
            this.receive = typeof receiveFunc === 'function' ? receiveFunc : ((type, value) => { })
        }
        _output(type, args) {
            let value = Object.prototype.valueOf.call(args)
            this.output_args.push({ type, value });
            this.receive(type, value)
            return this
        }
        error(...args) { this._output('error', args); return this }
        warn(...args) { this._output('warn', args); return this }
        info(...args) { this._output('info', args); return this }
        success(...args) { this._output('success', args); return this }
        tip(...args) { this._output('tip', args); return this }
        note(...args) { this._output('note', args); return this }
        log(...args) { this._output('log', args); return this }
        exit(...args) { throw new Error('exit') }
    }
    ...
    function asm2bin(input_asm_lines, options = {
        ...
        const getRegAddr = str => {
            ...
            if (/^\$[a-z0-9]+$/i.test(str)) {
                ...
                if (REG_MAP[param]) regpos = REG_MAP[param]
                    ...
                    that.io.log(`Assigned \$${i} to \$${param}`)
                }
                if (regpos > (1 << 5) - 1) {
                    if (ENABLE_WARN) that.io.warn(`Line ${CurFileLine}: Register ${str}${String(regpos) === param ? '' : ` (assigned \$${regpos})`} is too large!`)
                    else that.io.error(`Line ${CurFileLine}: Register ${str} is too large`).exit(1)
                }
                return getBinStr(regpos, 5)
            } else that.io.error(`Line ${CurFileLine}: Invalid register: ${str}`).exit(1)
        }
        ...
        const joinBit32 = ({ opcode, rs, rt, rd, shamt, funct, imm, addr }) => {
            ...
            if (res.length !== 32) that.io.error(`Line ${CurFileLine}: Invalid bit32 length: ${res}`).exit(1)
            return res
        }
        ...
        const parseAsmNum = (asm_num_string) => {
            ...
            that.io.error(`Line ${CurFileLine}: Invalid number format: ${asm_num_string}`).exit(1)
        }
        ...
        const getBinStr = (strOrNum, width, warn = ENABLE_WARN) => {
            ...
            if (res.length > width) {
                if (warn) that.io.warn(`Line ${CurFileLine}: Number ${str} is too large, will be truncated`)
                else that.io.error(`Line ${CurFileLine}: Number ${str} is too large`).exit(1)
            }
        ...
        const _gen = {
            ...
            IType_lw_sw(opcode) {
                return function (op1, op2) {
                    ...
                    if (!regexpres) that.io.error(`Line ${CurFileLine}: Invalid format: ${op2}`).exit(1)
                ...
                return function (op1, op2, op3) {
                    ...
                    // label
                        ...
                        if (typeof jump_table[op3] === "undefined")
                            that.io.error(`Line ${CurFileLine}: Label ${op3} not found`).exit(1)
                    ...
                    // label with offset (label+offset or label-offset)
                        ...
                        if (typeof jump_table[_label] === "undefined")
                            that.io.error(`Line ${CurFileLine}: Label ${_label} not found`).exit(1)
                        if ((jump_table[_label] >> 2) + _offset_num < 0) {
                            if (ENABLE_WARN) that.io.warn(`Line ${CurFileLine}: Address may be negative for ${op3}`)
                            else that.io.error(`Line ${CurFileLine}: Address may be negative for ${op3}`).exit(1)
                        }

                        offset = ((jump_table[op3] - CurPC - 4) >> 2) + _offset_num
                    }
                    // Exception
                    else that.io.error(`Line ${CurFileLine}: Invalid format: ${op3}`).exit(1)
                    return joinBit32({
                        opcode: opcode,
                        rs: getRegAddr(op1),
                        rt: getRegAddr(op2),
                ...
                return function (op1) {
                    ...
                    if (/^[a-zA-Z_][a-zA-Z_\(\)\d]*$/i.test(op1)) {
                        if (typeof jump_table[op1] === "undefined")
                            that.io.error(`Line ${CurFileLine}: Label ${op1} not found`).exit(1)
                    ...
                    // label with offset (label+offset or label-offset)
                        ...
                        if (typeof jump_table[_label] === "undefined")
                            that.io.error(`Line ${CurFileLine}: Label ${_label} not found`).exit(1)
                        if ((jump_table[_label] >> 2) + _offset_num < 0) {
                            if (ENABLE_WARN) that.io.warn(`Line ${CurFileLine}: Address may be negative for ${op1}`)
                            else that.io.error(`Line ${CurFileLine}: Address may be negative for ${op1}`).exit(1)
                        }
                    ...
                    // Exception
                        ...
                        try {
                            ...
                            that.io.error(`Line ${CurFileLine}: Invalid format: ${op1}`).exit(1)
                        }
                    }
        ...
        that.io.log(`Start PC: 0x${opts.startPC.toString(16).toUpperCase().padStart(8, '0')}`)
        ...
        for (let i = 0; i < tmpLines.length; i++) {
            ...
            if (fmtLine.includes(':')) {
                that.io.error(`Line ${i + 1}: Invalid label ${fmtLine}`).exit(1)
            }
        ...
        for (let i = 0; i < asm_lines.length; i++) {
            ...
            if (!VALID_ASM_OP_ARR.includes(asmOp)) {
                that.io.error(`Line ${CurFileLine}: Invalid opcode: ${asmOp}`).exit(1)
            }
    ...
    class ASMTransformer {
        constructor() {
            this._callback = (type, value, span) => { }
            this.io = new ConsoleIO((type, value) => this._callback(type, value, getIOSpan(type, value)))
            this.asm2bin = asm2bin.bind(this)
            this.bin2hex = bin2hex.bind(this)
        }

        /**
         * @param {(type, value, span) => any} callback
         */
        receive(callback) {
        ...
        getOutputSpan() {
            return this.io.output_args.map(t => {
                return getIOSpan(t.type, t.value)
            })
        }
    }

    window.ASMTransformer = ASMTransformer
}()